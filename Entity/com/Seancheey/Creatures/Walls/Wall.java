...
public class Wall extends Creature {
	...
	public void collisionOperation(int id) {
		...
		if (Math.abs(Game.map.getCreatureList().get(id).getCenterPosition().x
				- getCenterPosition().x) <= (double) (Game.map
				.getCreatureList().get(id).getWidth() + getWidth()) / 2) {

			Game.map.getCreatureList().get(id)
					.setVx(Game.map.getCreatureList().get(id).getVx() * -1);
		} else
		// if collide vertically, vy*=-1
		if (Math.abs(Game.map.getCreatureList().get(id).getCenterPosition().y
				- getCenterPosition().y) <= (double) (Game.map
				.getCreatureList().get(id).getHeight() + getHeight()) / 2) {

			Game.map.getCreatureList().get(id)
					.setVy(Game.map.getCreatureList().get(id).getVy() * -1);
		}
	}
}
