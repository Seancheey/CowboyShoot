...
public abstract class Entity implements RefreshAction {
	...
	public void paint(Graphics g) {
		if (HP > 0) {
			...
			g2d.rotate(rotation, GuiTool.fitWidth(getCenterPosition().x),
					GuiTool.fitHeight(getCenterPosition().y));
			g.drawImage(image, GuiTool.fitWidth((int) px),
					GuiTool.fitHeight((int) py), GuiTool.fitWidth(width),
	...
	protected void setImage(String imagePath) {
		...
		if (image.toString() == null)
			System.out
					.println("com.Seancheey.MOB---setImage---image is not found");
	}
